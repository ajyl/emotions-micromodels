...
def _get_segments(utterance: str, window_size: int, step_size: int) -> List[str]:
    """
...
#def _get_segments_tree(utterance: str, window_size) -> List[str]:
#    """
#    Break down utterance into segments based on constituency parse tree.
#
#    :param utterance: input query.
#    :window_size: size of each segment.
#    :step_size: step size for the sliding window.
...
#    # phrase,
#    # phrase_label
#    tree, _ = PARSER.get_parse_tree_for_raw_sent(utterance)
#    segments = [x["phrase"] for x in tree if x["ngram"] >= window_size]
#    return segments
...
class BertFeaturizer(Orchestrator):
    ...
    def setup(self):
        ...
        """
        self.seeds = {
            config["name"]: config["setup_args"]["seed"] for config in self.configs
        }
        self.config_idxs = {}
        ...
        for config in self.configs:
            ...
            segment_config = tuple(
                sorted(config["setup_args"]["infer_config"]["segment_config"].items())
            )
        ...
        utt_idx = 0
        curr_idxs = {
            _seg_config_idx: 0 for _seg_config_idx in self.config_idxs_inv.keys()
        }
        utt_idx2mm = {}
        # self.seed_idxs = {}
        ...
        for mm_name, seed in self.seeds.items():
            ...
            # self.seed_idxs[mm_name] = {
            #    "start": curr_idx,
            #    "end": curr_idx + len(seed) - 1,
            #    "seed": seed,
            #    "segment_config_id": config_id,
            # }
    ...
    def run_bert(self, queries: List[str]):
        ...
        for config_id, mm_objs in self.seed_group.items():
            ...
            query_embeddings = self.encode([query[1] for query in query_idx_map])
    ...
    def apply_threshold(self, bert_results, threshold):
        ...
        for emotion, results in bert_results.items():
            for utt_idx, result_obj in results.items():
                hit = 0
