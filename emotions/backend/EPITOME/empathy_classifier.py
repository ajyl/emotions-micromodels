...
class EmpathyClassifier:
    def __init__(
        ...
        ER_weights = torch.load(ER_model_path, map_location=torch.device("cpu"))
        self.model_ER.load_state_dict(ER_weights)

        IP_weights = torch.load(IP_model_path, map_location=torch.device("cpu"))
        self.model_IP.load_state_dict(IP_weights)

        EX_weights = torch.load(EX_model_path, map_location=torch.device("cpu"))
