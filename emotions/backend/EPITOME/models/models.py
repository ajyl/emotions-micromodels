...
class BiEncoderAttentionWithRationaleClassification(nn.Module):
    ...
    def forward(
        ...
        sequence_output_RP = sequence_output_RP + self.dropout(
            self.attn(
                sequence_output_RP, sequence_output_SP, sequence_output_SP
            )
        )

        logits_empathy = self.empathy_classifier(
            sequence_output_RP[:, 0, :]
        ...
        if rationale_labels is not None:
            ...
            if attention_mask_RP is not None:
                ...
                loss_rationales = loss_fct(
                    logits_rationales.view(-1, self.rationale_num_labels),
                    rationale_labels.view(-1),
                )
        ...
        if empathy_labels is not None:
            ...
            loss_empathy = loss_fct(
                logits_empathy.view(-1, self.empathy_num_labels),
                empathy_labels.view(-1),
            )
...
class RobertaClassificationHead(nn.Module):
    ...
    def __init__(
        self, hidden_dropout_prob=0.1, hidden_size=768, empathy_num_labels=3
    ):
        super().__init__()
