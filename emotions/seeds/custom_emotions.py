ED_SEEDS = {
    #"sentimental": [
    #    "I got sentimental",
    #],
    "afraid": [
        "I was very scared",
        "That was terrifying",
        "I got really scared",
        "I'm scared of",
    ],
    "proud": [
        "I am so proud of"
    ],
    "faithful": [
        "I've always been loyal",
        "I am faithful to",
    ],
    #"terrified": [
    #    "I was terrified by",
    #],
    "joyful": [
        "I am very happy",
        "It makes me so happy",
        "I am full of joy",
    ],
    "angry": [
        "I got so mad",
        "I am so angry at",
        "I am upset about"
    ],
    "sad": [
        "I cried when",
        "I was sad when",
        "I felt so heartbroken",
    ],
    "jealous": [
        "I wish I were",
        "I felt jealous",
        "I really wanted it",
        "I am so envious of",
    ],
    "grateful": [
        "I am grateful for",
        "I am so thankful for",
        "I am honored that",
        "I am so lucky to",
        "I am so glad that",
    ],
    "prepared": [
        "I am ready for",
        "I know what to expect"
    ],
    "embarrassed": [
        "I am embarrassed about",
        "it was so embarrassing",
    ],
    "excited": [
        "I am excited for"
    ],
    "annoyed": [
        "he annoys me",
        "I was so annoyed by",
        "I was rather peeved",
        "I was annoyed by",
        "got on my nerves",
        "I was ticked off",
    ],
    "lonely": [
        "My girlfriend broke up",
        "my marriage broke up",
        "got a divorce",
        "I am so alone",
        "my friends all left me",
        "I feel lonely",
        "I was always lonely",
        "I am all alone",
        "I have no friends"
    ],
    "ashamed": [
        "I was ashamed when",
        "I feel so bad about",
        "I lied to my",
        "I felt really bad about",
    ],
    "guilty": [
        "I feel guilty about",
        "I feel horrible about",
        "I feel terrible for",
    ],
    "surprised": [
        "really surprised me",
        "I was surprised with",
        "I am shocked",
        "he had no idea what was coming",
    ],
    "nostalgic": [
        "I really miss back when",
        "I felt remembering the old",
        "I get very nostalgic",
        "I miss my old days",
        "I was remembering the first",
    ],
    "confident": [
        "I felt confident that",
        "I was very confident",
        "felt very calm and collected",
        "I know that I'll be able to",
        "I am certain that I can do it"
    ],
    "furious": [
        "I was furious when",
        "that pissed me off",
        "I am so mad",
    ],
    "disappointed": [
        "I was disappointed at",
        "I was really disappointed to hear",
        "I was supposed to but then",
        "I tried but it wasn't enough",
        "the results were not good enough",
        "It was a let down",
        "Unfortunately I was not able to",
    ],
    "caring": [
        "I love taking care of",
        "I deeply love my",
    ],
    "trusting": [
        "I trust my",
    ],
    "disgusted": [
        "I am disgusted by",
        "it was gross",
        "made me want to vomit",
        "it was nasty",
    ],
    "anticipating": [
        "I can't wait for ",
        "I can hardly wait",
        "I am expecting to get",
        "We made big plans for",
    ],
    "anxious": [
        "I got so nervous about",
        "I am anxious that",
        "I am so nervous",
        "I always feel anxious",
        "I get so stressed when"
        "makes me nervous",
        "I am worried about",
    ],
    "hopeful": [
        "I am hopeful that",
        "I am optimistic about",
    ],
    "content": [
        "I am ok with ",
        "I came to terms with",
        "I feel at peace about",
        "I feel content with",
        "I feel satisfied with"
    ],
    "impressed": [
        "I was impressed by",
        "it was really impressive",
        "they turned out better than I expected",
    ],
    "apprehensive": [
        "I was apprehensive about",
    ],
    "devastated": [
        "I was devastated when",
        "it was tragic and difficult",
        "I was horrified",
        "My world fell apart",
        "it broke my heart",
        "I was completely heartbroken",
        "I am at a loss for words"
    ],
}
