...
class CrossScorer(nn.Module):
    ...
    def __init__(
        self, p_encoder, r_encoder, use_aux_loss=False
    ):  # , tokenizer):
        """"""
    ...
    def cl_loss(self, z1, z2, hn_batch=None, hn_output=None):
        ...
        if hn_batch:
            ...
            weights = torch.tensor(
                [
                    [0.0] * (cos_sim.size(-1) - p_hn_cos.size(-1))
                    + [0.0] * i
                    + [self.hn_weight]
                    + [0.0] * (p_hn_cos.size(-1) - i - 1)
                    for i in range(p_hn_cos.size(-1))
                ]
            ).to(self.device)
    ...
    def forward(
        ...
        if hn_batch:
            hn_output = self.r_encoder.emb_forward(**hn_batch)
    ...
    def cl_loss_with_hn(self, prompts, responses):
        ...
        for prompt, response_set in zip(prompts, responses):
            ...
            label = torch.LongTensor([0]).to(
                self.device
            )  # torch.arange(cos_sim.size(0)).long().to(self.device)
...
class CrossScorerCrossEncoder(nn.Module):
    ...
    def cl_loss_all_random(self, pair_scores, labels):
        ...
        hq_scores = pair_scores[
            :, 0
        ]  # .repeat(1,neg_scores.size(-1)).flatten()
        # 6
    ...
    def forward(
        ...
        if True:
            ...
            label[1] = 0.5
            labels = (
                torch.cat([label for x in range(BSZ)], -1)
                .float()
                .to(self.device)
            )
            ...
            if True:
                ...
                idx = np.array(
                    [i for i in range(len(pair_scores)) if i % 5 != 4]
                )
    ...
    def saved_forward(
        ...
        label[1] = 0.5
        labels = (
            torch.cat([label for x in range(BSZ)], -1).float().to(self.device)
        )
...
class CrossScorerBiEncoder(nn.Module):
    ...
    def score_forward(self, p_batch=None, r_batch=None):

        p_output = self.p_encoder(**p_batch)

        r_output = self.r_encoder(**r_batch)
        p_pooled = p_output.last_hidden_state[:, 0, :].unsqueeze(
            0
        )  # .transpose(1,0)
    ...
    def forward(self, p_batch=None, r_batch=None, random=False):
        """
        for just regression
        """

        pair_scores = self.score_forward(p_batch, r_batch).squeeze()
    ...
    def saved_forward(self, p_batch=None, r_batch=None, random=False):
    ...
    def hard_forward(self, p_batch=None, r_batch=None):

        return self.forward(p_batch, r_batch)
...
class CrossScorerWithHead(nn.Module):
    ...
    def forward(self, p_batch=None, r_batch=None, labels=None):

        p_output = self.p_encoder.emb_forward(**p_batch)

        r_output = self.r_encoder.emb_forward(**r_batch)
